...
class Importer {
    ...
    */
    public function import( $content, $roomID, $source_id, $source_title, $time ) {
        ...
        $sync_settings = apply_filters( 'loftocean_ical_cron_get_settings', array() );
        $import_bookings_without_validation = isset( $sync_settings[ 'import_bookings_without_availability_checking' ] ) && ( 'on' == $sync_settings[ 'import_bookings_without_availability_checking' ] );
        try {
            ...
            if ( count( $events ) > 0 ) {
                ...
                if ( \LoftOcean\is_valid_array( $events_to_add ) ) {
                    ...
                    foreach ( $events_to_add as $event ) {
                        if ( $import_bookings_without_validation || apply_filters( 'loftocean_room_reservation_check_dates', false, $roomID, $event[ 'checkIn' ], $event[ 'checkOut' ], 1 ) ) {
