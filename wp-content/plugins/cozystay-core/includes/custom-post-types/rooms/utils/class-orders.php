	...
	class Orders {
        ...
        */
        public function __construct() {
			...
			add_filter( 'woocommerce_json_search_found_products', array( $this, 'exclude_variable_products' ), 99, 1 );
		}
		/*
		* Register REST APIs
		*/
					...
					if ( in_array( $order_status, $check_status ) ) {
						...
						foreach ( $items as $item ) {
							...
							if ( ( ! empty( $room_id ) ) && in_array( $room_id, $rids ) ) {
								$data = \LoftOcean\get_room_product_variation_data( $item->get_variation_id(), 'data' );
								if ( ! \LoftOcean\is_valid_array( $data ) ) continue;
								...
								if ( ( ( $start <= $checkin ) && ( $checkin <= $end ) ) || ( ( $start <= $checkout ) && ( $checkout <= $end ) ) || ( ( $checkin < $start ) && ( $checkout > $end ) ) ) {
									...
									$room_booking_detail = \LoftOcean\get_room_product_variation_data( $item->get_variation_id(), 'data' );
        ...
        */
        ...
        /*
        * Exclude variable produts for product ajax searching
        */
        public function exclude_variable_products( $products ) {
        	if ( \LoftOcean\is_valid_array( $products ) ) {
	        	$room_products = new \WP_Query( array( 'post_type' => 'product', 'posts_per_page' => -1, 'offset' => 0, 'fields' => 'ids', 'meta_query' => array( array( 'key' => '_loftocean_booking_id', 'compare' => 'EXISTS' ) ) ) );
	        	if ( $room_products->have_posts() ) {
		        	$parent_product_ids = $room_products->posts; 
		        	foreach ( $parent_product_ids as $ppi ) {
		        		if ( isset( $products[ $ppi ] ) ) { 
		        			unset( $products[ $ppi ] );
		        		}
		        		$room_variable_product_ids = \wc_get_products( array( 'limit' => -1, 'offset' => 0, 'return' => 'ids', 'parent' => $ppi, 'type' => 'variation' ) ); 
		        		if ( \Loftocean\is_valid_array( $room_variable_product_ids ) ) {
		        			foreach( $room_variable_product_ids as $rvpi ) {
		        				if ( isset( $products[ $rvpi ] ) ) {
		        					unset( $products[ $rvpi ] ); 
		        				}
		        			}
		        		}
