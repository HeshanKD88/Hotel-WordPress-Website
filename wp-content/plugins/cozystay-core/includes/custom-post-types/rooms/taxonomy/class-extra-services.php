                        ...
                        foreach ( $extra_services as $es ) {
                            ...
                            if ( empty( $term_id ) ) {
                                ...
                                if ( ! is_wp_error( $new_term ) && \LoftOcean\is_valid_array( $new_term ) ) {
                                    ...
                                    update_term_meta( $new_term_id, 'description', sanitize_text_field( $es[ 'description' ] ) );
                                }
                            } else {
                                ...
                                update_term_meta( $term_id, 'description', sanitize_text_field( $es[ 'description' ] ) );
                            }
                            $priority ++;
                        }
                    }
            ...
            if ( ( ! is_wp_error( $terms ) ) && \LoftOcean\is_valid_array( $terms ) ) {
                return array_map( function( $item_id ) {
                    ...
                    return array_merge( $term, array(
                        ...
                        'description' => get_term_meta( $item_id, 'description', true )
                    ) );
                }, $terms );
            }
            return array();
