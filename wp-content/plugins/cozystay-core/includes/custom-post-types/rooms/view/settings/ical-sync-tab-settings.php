<div class="cs-tab-content-wrapper" id="loftocean-room-ical-sync-tab-settings">
    <form id="room-ical-form" method="post" action="<?php echo esc_url( admin_url( 'edit.php?post_type=loftocean_room&page=loftocean_room_ical_sync_settings' ) ); ?>"><?php
        ...
        <h2><?php esc_html_e( 'Data Import Settings', 'loftocean' ); ?></h2>
        <table class="form-table">
            <tbody>
                <tr>
                    <th><?php esc_html_e( 'Import all data?', 'loftocean' ); ?></th>
                    <td>
                        <label>
                            <input type="checkbox" name="import_bookings_without_availability_checking" value="on" <?php checked( 'on', $settings[ 'import_bookings_without_availability_checking' ] ); ?>>
                            <?php printf(
                                // translator: 1/2 html tag
                                esc_html__( 'Import all external bookings without checking the number of rooms remaining on this site (%1$smore details about this option%2$s)', 'loftocean' ),
                                '<a target="_blank" href="https://loftocean.com/doc/cozystay/ptkb/some-external-bookings-were-not-imported/">',
                                '</a>'
                            ); ?>
                        </label>
                    </td>
