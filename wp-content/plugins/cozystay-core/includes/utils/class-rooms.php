    ...
    class Rooms {
        ...
        */
        public function __construct() {
            ...
            add_action( 'template_redirect', array( $this, 'room_search_result_page_redirect' ), 1 );
            ...
            add_filter( 'loftocean_get_room_permalink_with_url_params', array( $this, 'single_room_permalink_args' ) );
            add_filter( 'loftocean_skip_room_search_result_page_if_only_one_room_found', array( $this, 'skip_search_result_page' ), 10, 1 );
            add_filter( 'post_type_link', array( $this, 'room_permalink' ), 999, 2 );
        }
        /**
        ...
        public function get_room_details( $details, $room_id ) {
            if ( 'loftocean_room' == get_post_type( $room_id ) ) {
                ...
                if ( ! isset( $this->rooms_cache[ $room_key ] ) ) {
                    ...
                    $list_gallery = get_post_meta( $room_id, 'loftocean_rooms_gallery_ids', true );
                    ...
                    $this->rooms_cache[ $room_key ] = apply_filters( 'loftocean_room_details', array(
                        ...
                        'listGallery' => empty( $list_gallery ) ? false : explode( ',',  $list_gallery ),
                        'roomSettings' => array(
                            ...
                            'enableCustomVariablePrices' => get_post_meta( $room_id, 'loftocean_room_enable_custom_variable_prices', true ),
                            'customVariablePrices' => get_post_meta( $room_id, 'loftocean_room_custom_variable_prices', true )
                        )
                    ) );;
                }
        ...
        */
        public function room_search_result_page_before_list() {
            add_filter( 'post_type_link', array( $this, 'single_room_permalink_args' ), 99, 2 );
        }
        /**
        * Actions after room search result page list content
        */
        public function room_search_result_page_after_list() {
            remove_filter( 'post_type_link', array( $this, 'single_room_permalink_args' ), 99, 2 );
        }
        /**
        * Add URL args to single room url
        */
        public function single_room_permalink_args( $link, $post = null ) {
        ...
        * Room external link
        */
        public function room_permalink( $permalink, $post ) { 
            if ( is_object( $post ) && isset( $post->post_type ) && ( 'loftocean_room' == $post->post_type ) && ( ( ! class_exists( '\Elementor\Plugin' ) ) || ( ! \Elementor\Plugin::$instance->editor->is_edit_mode() ) ) ) {
                $url = get_post_meta( $post->ID, 'loftocean_room_url', true );
                $url = trim( $url ); 
                return empty( $url ) ? $permalink : $url; 
            }
            return $permalink;
        }
        /*
        * Check if skip room search result page
        */
        public function skip_search_result_page( $skip ) {
            return ( 'on' == get_option( 'loftocean_skip_room_search_results_page', '' ) );
        }
        /**
        * Room search result page redirect if only one result found
        */
        public function room_search_result_page_redirect() {
            if ( ( ! is_admin() ) && isset( $_GET[ 'search_rooms' ] ) && apply_filters( 'loftocean_skip_room_search_result_page_if_only_one_room_found', false ) && have_posts() ) {
                global $wp_query;
                if ( 1 === $wp_query->found_posts ) {                    
                    the_post();
                    $url = apply_filters( 'loftocean_get_room_permalink_with_url_params', get_the_permalink() );
                    wp_redirect( $url );
                    die;
                }
            }
