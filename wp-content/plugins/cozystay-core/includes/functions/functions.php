...
function get_special_price_rate( $list, $date ) { 
	$current_found_rate = -1;
    $current_start_date = '';
    $current_end_date = '';

    foreach ( $list as $item ) {
        $custom_time_range = ( 'all' != $item[ 'id' ] );
        $start_date = $custom_time_range ? $item[ 'start' ] : '';
        $end_date = $custom_time_range ? $item[ 'end' ] : '';
        $is_numeric_start_date = is_numeric( $start_date );
        $is_numeric_end_date = is_numeric( $end_date );

        if ( ( ! $custom_time_range ) || ( ( ( ! $is_numeric_start_date ) || ( $start_date <= $date ) ) && ( ( ! $is_numeric_end_date ) || ( $end_date >= $date ) ) ) ) {
            if ( $current_found_rate < 0 ) {
                $current_found_rate = $item[ 'rate' ];
                $current_start_date = $start_date;
                $current_end_date = $end_date;
            } else if ( $custom_time_range && ( $is_numeric_start_date || $is_numeric_end_date ) && ( ( $current_start_date . '-' . $current_end_date ) !== ( $start_date . '-' . $end_date ) ) ) {
                ...
                if ( ( ! $is_numeric_current_start_date ) && ( ! $is_numeric_current_end_date ) && ( $is_numeric_start_date || $is_numeric_end_date ) ) {
					$current_found_rule_id = $rule_id;
                    $current_start_date = $start_date;
                    $current_end_date = $end_date;
                } else if ( ( ( ! $is_numeric_start_date ) && ( ! $is_numeric_current_start_date ) && $is_numeric_current_end_date && $before_current_end_date ) 
    ...
    return is_numeric( $current_found_rate ) && ( $current_found_rate > -1 ) ? $current_found_rate : 1;
}
/**
...
*/
function get_tax_location_for( $order = null ) {
	$tax_base = get_option( 'woocommerce_tax_based_on', 'shipping' );
	if ( ( null === $order ) || ( 'WC_Order' != get_class( $order ) ) || ( ! in_array( $tax_base, array( 'billing', 'shipping' ) ) ) ) {
		return array(
			'country' => \WC()->countries->get_base_country(),
			'state' => \WC()->countries->get_base_state(),
			'postcode' => \WC()->countries->get_base_postcode(),
...
function get_room_booked_status() {
	return apply_filters( 'loftocean_room_booked_status', array( 'processing', 'completed', 'on-hold', 'partially-paid' ) );
}
/**
* Room available status
*/
function get_room_available_status() {
	return apply_filters( 'loftocean_room_available_status', array( 'pending', 'cancelled', 'refunded', 'failed', 'checkout-draft' ) );
}
/**
* Get room bookings
*/
...
* Get matched rule id
*/
function get_matched_rule_item( $rules, $date ) {
    $current_found_rule_id = -1;
    $current_start_date = '';
    $current_end_date = '';

    foreach ( $rules as $rule_id ) {
        $custom_time_range = ( 'custom' == get_term_meta( $rule_id, 'time_range', true ) );
        $start_date = $custom_time_range ? get_term_meta( $rule_id, 'start_date', true ) : '';
        $end_date = $custom_time_range ? get_term_meta( $rule_id, 'end_date', true ) : '';
        $is_numeric_start_date = is_numeric( $start_date );
        $is_numeric_end_date = is_numeric( $end_date );

        if ( ( ! $custom_time_range ) || ( ( ( ! $is_numeric_start_date ) || ( $start_date <= $date ) ) && ( ( ! $is_numeric_end_date ) || ( $end_date >= $date ) ) ) ) {
            if ( $current_found_rule_id < 0 ) {
                $current_found_rule_id = $rule_id;
                $current_start_date = $start_date;
                $current_end_date = $end_date;
            } else if ( $custom_time_range && ( $is_numeric_start_date || $is_numeric_end_date ) && ( ( $current_start_date . '-' . $current_end_date ) !== ( $start_date . '-' . $end_date ) ) ) {
    ...
    return ( $current_found_rule_id > -1 ) ? $current_found_rule_id : false;
}
/*
* Update room product variation data
*/
function update_room_product_variation_data( $variation_id, $data, $name = 'data' ) {
	if ( empty( $name ) ) return ;

	update_post_meta( $variation_id, $name, $data );

	if ( is_array( $data ) ) {
		$data = json_encode( $data );
	}
	$data = base64_encode( $data );
    update_post_meta( $variation_id, 'serialized_' . $name, $data );

	
}
/*
* Get room product variantion data
*/
function get_room_product_variation_data( $variation_id, $name = 'data' ) {
	if ( empty( $name ) ) return false;

	$data = get_post_meta( $variation_id, 'serialized_' . $name, true );
    if ( empty( $data ) ) {
    	return get_post_meta( $variation_id, $name, true );
    } else { 
    	if ( is_array( $data ) ) {
			return $data;
		}
...
*/
function update_order_roomIDs( $order ) {
	$order->delete_meta_data( '_loftocean_order_room_ID' );
