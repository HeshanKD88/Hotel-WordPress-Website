( function( $ ) {
    ...
    var $reservationForm, $pageContainer, $extraServices, $totalPrice, $roomMessage, $errorMessage, $successMessage, $loading, $priceDetails, $basePrice, 
		...
		variablePriceEnabled = false, variablePrices = {}, variableWeekendPriceEnabled = false, groupGuests = false, currentPriceList = false, currentPriceListStamp = false;

	function updateLowestPrice() {
		var lowest = false, originalLowest = false;

		generateCurrentPriceList();
		if ( false !== currentPriceList ) {
			for ( var i = checkinTimestamp; i < checkoutTimestamp; i += dayTime ) {
				if ( currentPriceList[ i ] && ( 'available' == currentPriceList[ i ][ 'status' ] ) ) {
					var currentActualPrice, currentPrices = currentPriceList[ i ],
					...
					currentActualPrice = currentPrices[ 'special_price_rate' ] ? multiplication( currentOriginalPrice, currentPrices[ 'special_price_rate' ] ) : currentOriginalPrice;
	...
	function updateRoomMessage() {
		...
		if ( ( ! failed ) && ( false !== lowest ) && ( lowest > 0 ) ) {
			...
			if ( roomNumber > lowest ) {
				...
				$roomNumber.val( lowest - 1 ).siblings( '.plus' ).removeClass( 'disabled' ).data( 'status', '' ).trigger( 'click' );
	...
	function updatePriceDetails() {
		...
		if ( false !== currentPriceList ) {
			for ( var i = checkinTimestamp; i < checkoutTimestamp; i += dayTime ) {
				if ( currentPriceList[ i ] ) {
					if ( 'available' == currentPriceList[ i ][ 'status' ] ) {
						var currentPrices = currentPriceList[ i ], rate = currentPrices[ 'special_price_rate' ] ? currentPrices[ 'special_price_rate' ] : 1,
    ...
    function generateCurrentPriceList() {
    	var checkedPriceListStamp = '' + checkinDate + '-' + checkoutDate + '-' + adultNumber + '-' + childNumber; 
    	if ( checkedPriceListStamp == currentPriceListStamp ) return;

    	currentPriceListStamp = checkedPriceListStamp;
    	currentPriceList = {};
    	for ( var i = checkinTimestamp; i < checkoutTimestamp; i += dayTime ) {
    		if ( priceList[ i ] && ( 'available' == priceList[ i ][ 'status' ] ) ) {
	    		var currentPrices = getCurrentRoomPrices( i );
    			currentPriceList[ i ] = $.extend( {}, priceList[ i ], currentPrices );
    		} else {
    			currentPriceList[ i ] = $.extend( {}, priceList[ i ] );
    		} 
    	}
    }
    function getCurrentRoomPrices( indexTime ) {
    	...
    	if ( loftoceanRoomReservation.variablePrices.enable ) { 
    		variablePriceEnabled ? checkVariablePricesItem( returnPrices, variablePrices, indexTime, groupGuests ) : '';

    		var currentCustomVariablePrices = checkCustomVariablePrices( indexTime );
    		if ( false !== currentCustomVariablePrices ) { 
    			checkVariablePricesItem( returnPrices, currentCustomVariablePrices.prices, indexTime, ( 'group' == currentCustomVariablePrices.guestMode ) );
    		}  
    	}
    	return returnPrices;
    }
    function checkCustomVariablePrices( currentDate ) {
    	var currentItemIndex = -1, currentStartDate = NaN, currentEndDate = NaN;
		if ( loftoceanRoomReservation.customVariablePrices && Array.isArray( loftoceanRoomReservation.customVariablePrices ) ) {
			loftoceanRoomReservation.customVariablePrices.forEach( function( cvp, index ) { 
				if ( cvp.hasOwnProperty( 'dateRange' ) && cvp.hasOwnProperty( 'prices' ) && cvp.hasOwnProperty( 'guestMode' ) && Array.isArray( cvp.dateRange ) ) {
					cvp.dateRange.forEach( function( dr ) { 
						var startDate = parseInt( dr[ 'start_date_timestamp' ], 10 ), endDate = parseInt( dr[ 'end_date_timestamp' ], 10 ),
							isStartDateNotSet = isNaN( startDate ), isEndDateNotSet = isNaN( endDate );

						if ( ( ( ! isStartDateNotSet ) || ( ! isEndDateNotSet ) ) && ( isStartDateNotSet || ( startDate <= currentDate ) ) && ( isEndDateNotSet || ( endDate >= currentDate ) ) ) { 
							if ( currentItemIndex < 0 ) { 
								currentItemIndex = index;
								currentStartDate = startDate;
								currentEndDate = endDate;
							} else if ( ( currentStartDate + '-' + currentEndDate ) !== ( startDate + '-' + endDate ) ) { 
								...
								if ( ( isStartDateNotSet && isCurrentStartDateNotSet && ( ! isCurrentEndDateNotSet ) && beforeCurrentEndDate ) 
		...
		return ( currentItemIndex > -1 ) ? loftoceanRoomReservation.customVariablePrices[ currentItemIndex ] : false;
    }
    function checkVariablePricesItem( returnPrices, priceItems, indexTime, enableGuestGroup ) {
	   	var guestNumber = getNumber( adultNumber ) + getNumber( childNumber ), 
	   		propertyNames = enableGuestGroup ? [ 'adult' + adultNumber + '-child' + childNumber, 'adult' + adultNumber + '-child', 'adult-child' + childNumber, 'adult-child' ] : [ 'guests' + guestNumber, 'guests' ];

		propertyNames.every( function( propertyName ) {
			if ( priceItems.hasOwnProperty( propertyName ) ) {
				var prices = priceItems[ propertyName ]; 
	...
	function getMatchedBookingRule( date ) { 
		if ( loftoceanRoomReservation.unavailableDates && loftoceanRoomReservation.unavailableDates.length ) {
			var currentItemIndex = -1, currentStartDate = NaN, currentEndDate = NaN;
			loftoceanRoomReservation.unavailableDates.forEach( function( item, index ) { 
				var notCustom = ( 'custom' != item[ 'type' ] ), 
					startDate = notCustom ? NaN : parseInt( item[ 'start' ], 10 ), endDate = notCustom ? NaN : parseInt( item[ 'end' ], 10 ),
					isStartDateNotSet = isNaN( startDate ), isEndDateNotSet = isNaN( endDate );

				if ( notCustom || ( ( isStartDateNotSet || ( startDate <= date ) ) && ( isEndDateNotSet || ( endDate >= date ) ) ) ) {
					if ( currentItemIndex < 0 ) {
						currentItemIndex = index;
						currentStartDate = startDate;
						currentEndDate = endDate;
					} else if ( ( 'custom' == item[ 'type' ] ) && ( ( ! isStartDateNotSet ) || ( ! isEndDateNotSet ) ) && ( ( currentStartDate + '-' + currentEndDate ) !== ( startDate + '-' + endDate ) ) ) {
						var isCurrentStartDateNotSet = isNaN( currentStartDate ), isCurrentEndDateNotSet = isNaN( currentEndDate ),
							afterCurrentStartDate = ( ! isStartDateNotSet ) && ( isCurrentStartDateNotSet || ( startDate >= currentStartDate ) ),
							beforeCurrentEndDate = ( ! isEndDateNotSet ) && ( isCurrentEndDateNotSet || ( endDate <= currentEndDate ) );
			...
			return ( currentItemIndex > -1 ) ? loftoceanRoomReservation.unavailableDates[ currentItemIndex ] : false;
		}
		return false;
	}
	...
	function checkDateAvailability( date, drp ) {
		...
		if ( ( typeof drp === 'undefined' || null === drp.startDate || null !== drp.endDate ) && disabledStartDates.includes( date ) ) {
			...
			var d = new Date( date ), dayOfWeek = 'day' + d.getDay(), currentTimstamp = getTimeStamp( d ), classes = [], messages = [],
				checkinRuleItem = false, currentDateRuleItem = getMatchedBookingRule( currentTimstamp );

			checkinRuleItem = notSetEndDateYet ? getMatchedBookingRule( getTimeStamp( new Date( drp.startDate.format( dateFormat ) ) ) ) : false;  
	...
	document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $adultNumber.on( 'loftocean.number.changed', function( e, isPlus, $btn ) {
            ...
            calculateExtraServiceTotalPrice();
            updateTotalPrice();
			if ( loftoceanRoomReservation.pricePerPerson || loftoceanRoomReservation.variablePrices.enable ) {
				updateLowestPrice();
			}
		...
		$( 'body' ).on( 'click', function( e ) {
			var $target = $( e.target ), $priceBreakdown = $( '.csf-base-price-breakdown' ), $extraServiceDescription = $( '.cs-service-description.show' );
			...
			if ( $extraServiceDescription.length && ( ! $target.hasClass( 'cs-service-description' ) ) && ( ! $target.parents( '.cs-service-description' ).length ) ) {
				$extraServiceDescription.removeClass( 'show' );
			}
		} );
		$reservationForm.on( 'click', '.csf-pd-total-base .csf-pd-label', function( e ) {
			...
			if ( $priceBreakdown.hasClass( 'show' ) ) { 
				$priceBreakdown.removeClass( 'show' );
			} else {
				$( '.cs-service-description.show' ).removeClass( 'show' );
				$priceBreakdown.addClass( 'show' );
			}
		} ).on( 'click', '.cs-form-field.cs-extra-service.has-description .info-indicator', function( e ) {
			e.stopImmediatePropagation();
			e.stopPropagation();

			var $description = $( this ).closest( '.cs-extra-service.has-description' ).find( '.cs-service-description' );
			if ( $description.length ) { 
				if ( $description.hasClass( 'show' ) ) { 
					$description.removeClass( 'show' );
				} else {
					$( '.cs-service-description.show, .csf-base-price-breakdown.show' ).removeClass( 'show' );
