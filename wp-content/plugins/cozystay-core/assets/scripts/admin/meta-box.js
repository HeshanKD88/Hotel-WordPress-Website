( function( $ ) {
	...
	$( document ).ready( function() {
		...
		wp.loftoceanMediaTools = function() {
			...
			wp.loftocean.roomsGallery = new wp.media( {
				frame: 'post',
				state: 'gallery-edit',
				title:	wp.media.view.l10n.editGalleryTitle,
				'media-sidebar': false,
			...
			wp.loftocean.roomsGallery.on( 'update', function( selection ) {
				var state = wp.loftocean.roomsGallery.state();
				selection = selection || state.get( 'selection' );
				...
				var gallery = wp.loftocean.roomsGallery.states.get( 'gallery-edit' ).get( 'library' ),
					$ul = $( '<ul>' );
				gallery.pluck( 'url' ).forEach( function( url ) {
					$ul.append( $( '<li>' ).append( $( '<a>', { 'href': '#', 'class': 'set-gallery' } ).append( $( '<img>', { 'src': url } ) ) ) );
				} );
				$media.find( '[name=loftocean_rooms_gallery_ids]' ).val( gallery.pluck( 'id' ).join( ',' ) );
				$media.find( '.gallery-no-image-wrapper' ).css( 'display', 'none' )
					.end().find( '.gallery-preview-list' ).html( '' ).append( $ul.children() )
					.end().find( '.gallery-has-image-wrapper' ).css( 'display', '' );

			} ).on( 'open', function() {
				var controller = wp.loftocean.roomsGallery.states.get( 'gallery-edit' ),
					library	= controller.get( 'library' ),
					ids  = $media.find( '[name=loftocean_rooms_gallery_ids]' ).val();
				if ( ids ) {
					ids = ids.split( ',' );
					ids.forEach( function( id ) {
						var attachment = wp.media.attachment( id );
		...
		if ( $listThumbnailBox.length ) {
			var $galleryBox = $( '#loftocean-room-list-gallery.postbox' ),
				$roomsGalleryBox = $( '#loftocean-rooms-gallery.postbox ' );
			...
			$roomsGalleryBox.on( 'click', '.set-gallery', function( e ) {
				e.preventDefault();
				$media = $roomsGalleryBox;
				roomMedia.roomsGallery.open();
			} ).on( 'click', '.remove-gallery', function( e ) {
				e.preventDefault();
				$galleryBox.find( '[name=loftocean_rooms_gallery_ids]' ).val( '' );
				$galleryBox.find( '.gallery-has-image-wrapper' ).css( 'display', 'none' )
					.end().find( '.gallery-no-image-wrapper' ).css( 'display', '' )
					.end().find( '.gallery-preview-list' ).html( '' );
			} );
