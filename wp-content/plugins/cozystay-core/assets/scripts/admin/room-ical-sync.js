( function( $ ) {
    ...
    if ( $calendars.length ) {
        var $bulkAction = $calendars.find( 'input.button.action.doaction' ),
            ...
            $importCalendarAddBtn = $import.find( 'input.button-primary' ),
        ...
        $importColumns.on( 'click', 'button.loftocean-ical-calendar-import', function( e ) {
            ...
            $importCalendarTitle.val( '' );
            $importCalendarIndex.val( '' );
            $importCalendarAddBtn.val( $importCalendarAddBtn.data( 'default-value' ) );
        ...
        .on( 'click', '.edit-item', function( e ) {
            ...
            $importCalendarAddBtn.val( $importCalendarAddBtn.data( $wrap.data( 'sync-url-base64' ) ? 'edit-value' : 'default-value' ) );
        } )
    ...
    if ( $importedBookings.length ) {
        ...
        } ).on( 'click', '.button.loftocean-delete-all-bookings', function( e ) {
            e.preventDefault();
            
            var $btn = $( this );
            $btn.attr( 'disabled', 'disabled' ).addClass( 'loading' );
            $.post( loftoceanRoomiCalSyncSettings.url, { 'action': loftoceanRoomiCalSyncSettings.actionRemoveAllBookings } ).always( function() {
